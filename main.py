...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not requested_user:
            new_user = User(name=form.name.data, password=password_hash, email=form.email.data)
            db.session.add(new_user)
            db.session.commit()
            login_user(new_user)
            return redirect(url_for("get_all_posts"))
        else:
            flash("User already exists. Plz login")
    ...
    if form.validate_on_submit():
        ...
        if user:
            if check_password_hash(user.password, form.password.data):
                ...
                flash("Incorrect Password.Plz provide correct password")
                return redirect(url_for("login"))
        else:
            flash("No such user exists. Please register yourself")
